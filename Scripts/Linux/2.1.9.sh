...
benchmark_number="2.1.9"
description="Ensure network file system server services are not in use"
is_installed=$(dpkg-query -W -f='${binary:Package}\t${Status}\t${db:Status-Status}\n' nfskernel-server | grep "installed" | wc -l)
is_enabled=$(  systemctl is-enabled nfs-server.service 2>/dev/null | grep 'enabled' | wc -l)
is_active=$( systemctl is-active nfs-server.service 2>/dev/null | grep '^active' | wc -l)
