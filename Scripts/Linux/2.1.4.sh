...
benchmark_number="2.1.4"
description="Ensure dns services are not in use"
is_enabled=$(systemctl is-enabled bind9.service 2>/dev/null | grep 'enabled' | wc -l)
is_active=$(systemctl is-active bind9.service 2>/dev/null | grep '^active' | wc -l)
is_installed=$(dpkg-query -W -f='${binary:Package}\t${Status}\t${db:Status-Status}\n' bind9 | grep "installed" | wc -l)
